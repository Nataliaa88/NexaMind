...
export async function POST(request: Request): Promise<NextResponse<ApiResponse>> {
  console.log('[chat/mix] POST called')

  let body: Body
  try {
    body = await request.json()
  } catch (err) {
    console.error('[chat/mix] JSON parse error', { err })
    return createError('Corps JSON invalide', 400)
  }

  const query = body?.query?.trim()
  if (!query) {
    console.warn('[chat/mix] missing or empty query')
  ...
  if (authError || !user) {
    console.error('[chat/mix] auth failed', { error: authError, bearerToken: Boolean(bearerToken) })
    return createError('Utilisateur non authentifié', 401)
  }

  const userId = user.id
  console.log('[chat/mix] authenticated user', { userId })

  try {
    console.log('[chat/mix] fetching document context')
    const documentContext = await getDocumentContextForUser(supabase, userId)

    const userPrompt = buildContextPrompt(query, documentContext, 'Mixte')
    const messages = [
      { role: 'system' as const, content: systemPrompts.Mixte },
      { role: 'user' as const, content: userPrompt },
    ]

    console.log('[chat/mix] calling OpenAI', {
      hasDocuments: documentContext.length > 0,
      queryLength: query.length,
    })

    const reply = await getOpenAIChatCompletion(messages)
    console.log('[chat/mix] OpenAI response received', { replyLength: reply.length })
    ...
    console.error('[chat/mix] request failed', {
      error: msg,
      isOpenAIError,
    ...
    return NextResponse.json(
      { error: `Recherche mixte échouée : ${msg}` },
